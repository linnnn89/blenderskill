...
```
1. CAM-Turntable at 3/4 height, 45° angle
...
When matching a photo/concept (`${COMMANDCODE_SKILL_DIR}/references/reference-image-match.md`):
...
| Final still | Render at hero `CAM-*` — compare to reference |
| Compare tool | `get_viewport_screenshot` or bpy render still |

Run `${COMMANDCODE_SKILL_DIR}/references/visual-match-checklist.md` before delivery.
...
- Organize cameras in `COL-Cameras`
...
| Reference image match | `${COMMANDCODE_SKILL_DIR}/references/reference-image-match.md` | Photo/concept attached |
| Visual match validation | `${COMMANDCODE_SKILL_DIR}/references/visual-match-checklist.md` | Before delivery |
