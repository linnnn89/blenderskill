...
- **Front + side** → optional silhouette × depth-profile guidance. This manual still builds the flat outline-trace foundation; for a true multi-view volumetric reconstruction, route the request to `08-reference-locked/reference-to-3d/MANUAL.md` instead of treating this manual as the full solution.
...
```bash
python3 ${COMMANDCODE_SKILL_DIR}/02-modeling/wireframe-to-3d/scripts/wireframe_analyzer.py <input.png> <output.json>
```
...
See `${COMMANDCODE_SKILL_DIR}/assets/v0.9.0-validation/03_aviator_wireframe_to_3d.webp` for what wireframe-to-3d produces alone (flat outline tracing) vs `04_aviator_chained_upgrade.webp` for what the chained orchestration produces (a Ray-Ban-style hero render).

**The router (`blender-skill`) should always plan for the chain** when the user asks for a "model of X" from a wireframe.
