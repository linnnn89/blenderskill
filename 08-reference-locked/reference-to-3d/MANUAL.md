...
If the user says the output does not match the templates/textures/wireframes, stop standard `blender-skill` generation and enter this workflow. Repeated plausible-but-wrong renders usually mean the missing step is **reference analysis**, not another modeling iteration.
...
```bash
python3 ${COMMANDCODE_SKILL_DIR}/08-reference-locked/reference-to-3d/scripts/template_analyzer.py   --image /path/to/front_reference.png   --out /tmp/front_analysis.json   --mode edges  # use bright_on_dark/dark_on_bright/alpha when cleaner
```
...
```bash
python3 ${COMMANDCODE_SKILL_DIR}/08-reference-locked/reference-to-3d/scripts/silhouette_validator.py   --reference /tmp/reference_mask.png   --render /tmp/render_mask.png   --out /tmp/validation.json
```
